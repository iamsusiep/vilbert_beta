...
def EvaluatingModel(args, task_cfg, device, task_id, batch, model, task_dataloader, task_losses, results, others):
    ...
    if task_cfg[task_id]['type'] == 'VL-classifier':
        ...
        for i in range(vil_logit.size(0)):
            results.append({'question_id':question_id[i].item(), 'answer':[prob.item() for prob in probs[i]], 'is_correct_rationale':is_correct_rationale[i].item(), 'target': target[i].item()})
