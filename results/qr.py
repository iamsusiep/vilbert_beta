import json
filedir = 'QR/VRA-/val_result.json'
with open(filedir) as f:
    data = json.loads(f.read())
#ppend({'question_id':question_id[i].item(), 'answer':[prob.item() for prob in probs[i]], 'target': target[i].item()})
count = 0
cor = 0
for x in data:  
    values = x['answer']    
    count += 1
    if values.index(max(values)) == x['target']:
        cor += 1

print("QR accuracy", cor/count)
